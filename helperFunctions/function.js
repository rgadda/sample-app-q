  ...
  await alpaca
    .createOrder({
      ...
      take_profit: {
        limit_price:
          side === "buy"
            ? parseFloat(price + target)
            : parseFloat(price - target)
      },
      stop_loss: {
        stop_price:
          side === "buy"
            ? parseFloat(price - price * riskFactor)
            : parseFloat(price + price * riskFactor),
        limit_price:
          side === "buy"
            ? parseFloat(price - price * riskFactor * 1.25)
            : parseFloat(price + price * riskFactor * 1.25)
      }
    })
    .then(() => {
...
const submitOrder = async (
  ...
  var prom = new Promise(async (resolve, reject) => {
    if (quantity > 0) {
      if (backToBack) {
        console.log(
          `-------- Executing backToBack Order ${stock}: ${attempt}---------`
        );
        await alpaca
          .getPosition(stock)
          .then(position => {
  ...
  _.forEach(orders, order => {
    promOrders.push(
      new Promise(async (resolve, reject) => {
        await alpaca.cancelOrder(order.id).catch(err => {
          console.log(err.error);
        });
        resolve();
      })
        ...
        alpaca
          ...
          .then(resp => {
            ...
            if (isOpen) {
              ...
              var timeToCloseInHours = _.floor(
                (openTime - currTime) / 1000 / 60 / 60,
                2
              );
              var timeToCloseInMins = _.floor(
                (openTime - currTime) / 1000 / 60
              );
              if (timeToCloseInHours > 0) {
                console.log(
                  timeToCloseInHours + " hours til next market open."
                );
              } else {
...
const closeAllPositions = async () => {
  await alpaca
    ...
    .then(async resp => {
      ...
      const pos = _.filter(
        resp,
        p => !_.includes(config.strategy.longTerm.symbol, p.symbol)
      );
